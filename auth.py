...
def access_token(uid: int) -> str:
    return jwt.encode({"sub": str(uid), "typ": "a", "exp": now() + ACCESS_TTL}, JWT_SECRET, algorithm="HS256")
...
async def current_user(cred: HTTPAuthorizationCredentials = Depends(bearer)):
    ...
    c = db(); u = c.execute("SELECT * FROM users WHERE id=? AND deleted_at IS NULL", (int(data["sub"]),)).fetchone()
