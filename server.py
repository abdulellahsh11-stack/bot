...
if(d.activated){{
  document.getElementById('m').innerHTML='✅ تم تفعيل الاشتراك — جارٍ العودة للوحة خلال ثوانٍ...';
  document.getElementById('back').style.display='inline-block';
  setTimeout(()=>location.href='/',3000);
}}else{{
  document.getElementById('m').textContent='حالة: '+JSON.stringify(d);
  document.getElementById('back').style.display='inline-block';
}}}}</script>
<a id=back href="/" style="display:none;margin-top:16px;padding:10px 22px;font-size:15px;border-radius:10px;
 background:linear-gradient(135deg,#ca8a04,#eab308);color:#fff;text-decoration:none">↩️ العودة للوحة المشترك</a>
</body></html>""")
